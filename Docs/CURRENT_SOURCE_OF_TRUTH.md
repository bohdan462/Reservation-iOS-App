...
25. **Reservation attachments (iOS UI today)** — Reservation Detail attachment UI is still **local-only** in behavior (`PhotosPicker`, local cache). **Not cross-device visible yet.** `AttachmentFeatureFlag.remoteUploadEnabled` remains **false**. Not safe as operational source of truth across devices or after reinstall until Slice D wires remote list/upload/download/delete. See [RESERVATION_ATTACHMENTS.md](./RESERVATION_ATTACHMENTS.md).
