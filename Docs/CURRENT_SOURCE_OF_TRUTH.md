...
25. **Reservation attachments (iOS today)** — Reservation Detail has **local-only** photo attachments (`ReservationAttachmentRecord` + `AttachmentFileStore` on device). **Not backend-synced in app yet.** Not safe as operational source of truth across devices or after reinstall. See [RESERVATION_ATTACHMENTS.md](./RESERVATION_ATTACHMENTS.md).
26. **Reservation attachments (backend — deployed)** — Backend `a2422d3` (plugin **0.5.5**, DB **1.12.0**): private storage + `tryzub_reservation_attachments` + staff-auth list/upload/patch/delete/content routes. **Deployed and production-smoked.** **No public image URLs** in API; direct image path **404** on production test; guest self-service must never expose attachments. Directory marker URL may return cached 200 text/html — host hardening recommended. **Next code slice: iOS Attachment Slice C** (DTO/API/cache only).
