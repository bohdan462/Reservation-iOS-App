...
25. **Reservation attachments (iOS UI — Slice D + E)** — Slice D at `d947721`: detail-scoped list/upload/download/delete. Slice E at `9d2784d`: staff-friendly rows, manage/details sheet, tag + note edit (shared PATCH / local-only SwiftData), fit-to-screen preview, manage delete, `deletedRemote` skipped by note signals. `AttachmentFeatureFlag.remoteUploadEnabled` is **true** (gated by reservation id + credentials). **Live/cross-device verification still open** — do not treat as production-verified until checklist passes. Old pre-sync **local-only** attachments may remain on the original device only; staff should **reattach important old images** if they need shared visibility. Build **11** tracked (`eed6530`); TestFlight build 11 submitted — next upload **build 12**. See [RESERVATION_ATTACHMENTS.md](./RESERVATION_ATTACHMENTS.md).
