...
| Reservation attachments | **Backend deployed** (`a2422d3`, production-smoked); **iOS C/D/E** `17a0bee` / `d947721` / `9d2784d`; `remoteUploadEnabled` **true**; build **11** tracked; **live verification open**; old local-only may need reattach ([RESERVATION_ATTACHMENTS.md](./RESERVATION_ATTACHMENTS.md)); **no public image URLs** |
...
5. **Reservation attachments** — backend **deployed + production-smoked** (`a2422d3`); iOS Slices C/D/E **done** (`17a0bee` / `d947721` / `9d2784d`); **next: live verification** + TestFlight **build 12**. See [RESERVATION_ATTACHMENTS.md](./RESERVATION_ATTACHMENTS.md).
