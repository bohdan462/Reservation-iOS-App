...
| [RESERVATION_ATTACHMENTS.md](./RESERVATION_ATTACHMENTS.md) | Reservation attachments: backend deployed; iOS Slice C done (`17a0bee`); Slice D next |
...
**Current root HEAD:** `17a0bee` on `audit-current-state` (iOS attachment Slice C foundation; Slice D next).
